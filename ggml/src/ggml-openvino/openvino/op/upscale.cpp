...
OutputVector translate_upscale(const NodeContext & context) {
    ...
    for (size_t axis = 0; axis < 2; ++axis) {
        const int64_t in_dim = static_cast<int64_t>(input_shape[axis]);
        const int64_t out_dim = static_cast<int64_t>(output_shape[axis]);
        if (in_dim != out_dim) {
            std::vector<int64_t> indices(static_cast<std::size_t>(out_dim));
            for (int64_t i = 0; i < out_dim; ++i) {
                indices[static_cast<std::size_t>(i)] = (i * in_dim) / out_dim;
            }
    ...
    if (input_shape[2] != output_shape[2] || input_shape[3] != output_shape[3]) {
        ...
        if (op_case == 2 && antialias) { // GGML_SCALE_MODE_BILINEAR with antialias
            const int64_t H_in = static_cast<int64_t>(input_shape[2]);
            const int64_t W_in = static_cast<int64_t>(input_shape[3]);
            const int64_t H_out = static_cast<int64_t>(output_shape[2]);
            const int64_t W_out = static_cast<int64_t>(output_shape[3]);

            const float pixel_offset = 0.5f;

            // Height projection: [H_out, H_in]
            std::vector<float> Wy_data(static_cast<std::size_t>(H_out * H_in), 0.0f);
            const float sf1 = static_cast<float>(H_out) / static_cast<float>(H_in);
            const float support1 = std::max(1.0f, 1.0f / sf1);
            const float invscale1 = 1.0f / support1;

            for (int64_t i1 = 0; i1 < H_out; ++i1) {
                const float y = (static_cast<float>(i1) + pixel_offset) / sf1;
                const int64_t y_min = std::max<int64_t>(static_cast<int64_t>(y - support1 + pixel_offset), 0);
                const int64_t y_max = std::min<int64_t>(static_cast<int64_t>(y + support1 + pixel_offset), H_in);
                float total_weight = 0.0f;
                for (int64_t sy = y_min; sy < y_max; ++sy) {
                    float diff = std::abs((static_cast<float>(sy) - y + pixel_offset) * invscale1);
                    float weight = std::max(1.0f - diff, 0.0f);
                    Wy_data[static_cast<std::size_t>(i1 * H_in + sy)] = weight;
                    total_weight += weight;
                }
                if (total_weight > 0.0f) {
                    for (int64_t sy = y_min; sy < y_max; ++sy) {
                        Wy_data[static_cast<std::size_t>(i1 * H_in + sy)] /= total_weight;
                    }
                }
            }

            // Width projection: [W_in, W_out]
            std::vector<float> Wx_data(static_cast<std::size_t>(W_in * W_out), 0.0f);
            const float sf0 = static_cast<float>(W_out) / static_cast<float>(W_in);
            const float support0 = std::max(1.0f, 1.0f / sf0);
            const float invscale0 = 1.0f / support0;

            for (int64_t i0 = 0; i0 < W_out; ++i0) {
                const float x = (static_cast<float>(i0) + pixel_offset) / sf0;
                const int64_t x_min = std::max<int64_t>(static_cast<int64_t>(x - support0 + pixel_offset), 0);
                const int64_t x_max = std::min<int64_t>(static_cast<int64_t>(x + support0 + pixel_offset), W_in);
                float total_weight = 0.0f;
                for (int64_t sx = x_min; sx < x_max; ++sx) {
                    float diff = std::abs((static_cast<float>(sx) - x + pixel_offset) * invscale0);
                    float weight = std::max(1.0f - diff, 0.0f);
                    Wx_data[static_cast<std::size_t>(sx * W_out + i0)] = weight;
                    total_weight += weight;
                }
                if (total_weight > 0.0f) {
                    for (int64_t sx = x_min; sx < x_max; ++sx) {
                        Wx_data[static_cast<std::size_t>(sx * W_out + i0)] /= total_weight;
                    }
                }
            }

            auto Wy_node = ov::op::v0::Constant::create(ov::element::f32, {static_cast<size_t>(H_out), static_cast<size_t>(H_in)}, Wy_data);
            auto Wx_node = ov::op::v0::Constant::create(ov::element::f32, {static_cast<size_t>(W_in), static_cast<size_t>(W_out)}, Wx_data);
